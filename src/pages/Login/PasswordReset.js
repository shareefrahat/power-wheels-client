...
const PasswordReset = () => {
  const [successful, setSuccessful] = useState({ send: false, email: "" });
  ...
  const handlePasswordReset = async (e) => {
    ...
    if (email) {
      ...
      if (error) {
        ...
        setSuccessful({ send: true, email: email });
      }
    } else {
      ...
      <section>
        <div className="card w-fit mx-auto bg-base-100 shadow-xl">
          {successful?.send ? (
            <div className="p-5">
              <p className="text-lg">
                Password reset email has been sent to <br />
                <span className="text-red-700">{successful?.email}</span>
              </p>
              <Link to="/login" className="btn btn-accent mt-5">
                Login
              </Link>
            </div>
          ) : (
